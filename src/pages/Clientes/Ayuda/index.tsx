import { Link } from 'react-router-dom'
import { Accordion, AccordionItem } from '@heroui/accordion'

const index = () => {
  return (
    <section className='fade-in p-4 md:p-6 2xl:p-10 flex items-start flex-col gap-y-6 max-w-5xl'>
      <h1 className='text-2xl lg:text-3xl font-bold text-primary'>Ayuda</h1>
      <p className='font-medium'>
        Sabemos que adaptarse a un nuevo sistema puede generar dudas, pero no te preocupes: estamos aquí para ayudarte.
        En esta sección encontrarás respuestas claras y sencillas a las consultas más comunes sobre el uso de nuestra
        plataforma. Nuestro objetivo es que este cambio sea fácil y eficiente. Si tu duda no se encuentra dentro de las
        siguientes preguntas, siempre podés{' '}
        <Link
          to='/contacto'
          className='underline text-primary hover:text-black'
        >
          contactarnos
        </Link>
        .
      </p>
      <h1 className='text-3xl font-bold mt-3'>Preguntas Frecuentes</h1>

      <Accordion className='max-w-3xl'>
        <AccordionItem
          key='1'
          aria-label='¿Cómo obtengo mi cuenta para acceder al sistema de pedidos?'
          title='¿Cómo obtengo mi cuenta para acceder al sistema de pedidos?'
          classNames={{
            title: 'font-bold'
          }}
        >
          La creación de nuevos usuarios debe gestionarse directamente con nuestra empresa. Contactanos para solicitar
          tu usuario y contraseña. Una vez que recibas estos datos de acceso, podrás iniciar sesión e ingresar al
          sistema las veces que quieras.
        </AccordionItem>
        <AccordionItem
          key='2'
          aria-label='¿Qué hago si olvido mi contraseña?'
          title='¿Qué hago si olvido mi contraseña?'
          classNames={{
            title: 'font-bold'
          }}
        >
          En caso de olvidar tu contraseña deberás ponerte en contacto con la empresa para restablecerla.
        </AccordionItem>
        <AccordionItem
          key='3'
          aria-label='¿Cómo realizar un pedido?'
          title='¿Cómo realizar un pedido?'
          classNames={{
            title: 'font-bold'
          }}
        >
          <div>
            - Ingresá a tu cuenta con tu usuario y contraseña.
            <br />- En el menú lateral, seleccioná la opción "Formulario de pedidos", donde encontrarás el detalle de
            los productos disponibles con sus respectivos códigos.
            <br />- Seleccioná la cantidad que necesitás de cada ítem.
            <br />- Elegí la fecha y horario de preferencia para el retiro del pedido.
            <br />
            Recordá que todos los pedidos deben ser aprobados por nuestro equipo. Antes de retirarlo, te sugerimos
            confirmar que ya esté preparado.
          </div>
        </AccordionItem>
        <AccordionItem
          key='4'
          aria-label='¿Puedo modificar un pedido ya realizado?'
          title='¿Puedo modificar un pedido ya realizado?'
          classNames={{
            title: 'font-bold'
          }}
        >
          No es posible modificar el pedido una vez realizado, pero podés contactarnos directamente para solicitar
          cualquier cambio.
        </AccordionItem>
        <AccordionItem
          key='5'
          aria-label='¿Cómo puedo seguir el estado de mis pedidos?'
          title='¿Cómo puedo seguir el estado de mis pedidos?'
          classNames={{
            title: 'font-bold'
          }}
        >
          - Ingresá a tu cuenta con tus datos de acceso. <br />
          - En el menú lateral, seleccioná la opción &quot;Historial de pedidos&quot; <br />- Allí podrás ver un listado
          de todos tus pedidos ordenados por fecha.
        </AccordionItem>
        <AccordionItem
          key='6'
          aria-label='¿Qué hago si tengo problemas técnicos al usar el sistema?'
          title='¿Qué hago si tengo problemas técnicos al usar el sistema?'
          classNames={{
            title: 'font-bold'
          }}
        >
          Asegurate de contar con una conexión a internet estable. Si el problema persiste, comunicate con nosotros para
          que podamos asistirte de manera rápida y efectiva.
        </AccordionItem>
      </Accordion>

      <h1 className='text-3xl font-bold mt-3'>¿Necesitas más ayuda? Contactanos</h1>
      <a
        href='https://wa.me/5493417464108'
        className='flex items-center gap-x-2 bg-primary text-white py-4 px-6 rounded-full hover:bg-opacity-90  transition-colors text-sm lg:text-base'
      >
        <svg
          xmlns='http://www.w3.org/2000/svg'
          viewBox='0 0 448 512'
          fill='currentColor'
          className='w-6 h-6'
        >
          <path d='M380.9 97.1C339 55.1 283.2 32 223.9 32c-122.4 0-222 99.6-222 222 0 39.1 10.2 77.3 29.6 111L0 480l117.7-30.9c32.4 17.7 68.9 27 106.1 27h.1c122.3 0 224.1-99.6 224.1-222 0-59.3-25.2-115-67.1-157zm-157 341.6c-33.2 0-65.7-8.9-94-25.7l-6.7-4-69.8 18.3L72 359.2l-4.4-7c-18.5-29.4-28.2-63.3-28.2-98.2 0-101.7 82.8-184.5 184.6-184.5 49.3 0 95.6 19.2 130.4 54.1 34.8 34.9 56.2 81.2 56.1 130.5 0 101.8-84.9 184.6-186.6 184.6zm101.2-138.2c-5.5-2.8-32.8-16.2-37.9-18-5.1-1.9-8.8-2.8-12.5 2.8-3.7 5.6-14.3 18-17.6 21.8-3.2 3.7-6.5 4.2-12 1.4-32.6-16.3-54-29.1-75.5-66-5.7-9.8 5.7-9.1 16.3-30.3 1.8-3.7 .9-6.9-.5-9.7-1.4-2.8-12.5-30.1-17.1-41.2-4.5-10.8-9.1-9.3-12.5-9.5-3.2-.2-6.9-.2-10.6-.2-3.7 0-9.7 1.4-14.8 6.9-5.1 5.6-19.4 19-19.4 46.3 0 27.3 19.9 53.7 22.6 57.4 2.8 3.7 39.1 59.7 94.8 83.8 35.2 15.2 49 16.5 66.6 13.9 10.7-1.6 32.8-13.4 37.4-26.4 4.6-13 4.6-24.1 3.2-26.4-1.3-2.5-5-3.9-10.5-6.6z' />
        </svg>
        <span>Whatsapp</span>
      </a>
    </section>
  )
}

export default index
